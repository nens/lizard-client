...
angular.module("omnibox").directive("omnibox", [
  "$window",
  "$document",
  "State",
  "user",
     ...
     */
        ...
        scope.showAnnotations = function() {
          return (
            ...
            scope.omnibox.data.geometries.length +
              scope.omnibox.data.assets.length <
              2
          );
        };
       ...
       */
        var setMaxHeight = function() {
          var tlHeight =
            $document.find("#timeline").height() + TIMLINE_BOTTOM_MARGIN;
          var maxHeight =
            $window.innerHeight -
            SEARCHBAR_FROM_TOP -
        ...
        scope.$watch(State.toString("context"), function(n) {
          if (n === "dashboard") {
            State.selections.forEach(function(selection) {
              var nextIdx = ChartCompositionService.getChartIndexForSelection(
                selection.uuid
              );
        ...
        scope.$watch(State.toString("geometries"), function(n, o) {
          ...
          o.forEach(function(str) {
            if (n.indexOf(str) === -1) {
              ...
              coordString =
                removedGeomObj.geometry.coordinates[0] +
                "," +
